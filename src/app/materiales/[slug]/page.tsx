...
export default async function MaterialDetailPage({ params }: Props) {
  ...
  const relatedMaterials = highlightedMaterials
  ...
  const relatedProjects = projects
    .filter(
      (project) =>
        project.material.includes(material.category) ||
        project.material.includes(material.name.split(' ')[0]),
    )
    .slice(0, 3);
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-2">
          <div className="grid gap-3">
            <div
              ...
              style={{
                backgroundImage: `linear-gradient(130deg, rgba(26,26,26,0.3), rgba(26,26,26,0.08)), radial-gradient(circle at 20% 20%, rgba(255,255,255,0.18), transparent 48%), url('${material.gallery[0]}')`,
            ...
            <div className="grid grid-cols-3 gap-3">
              {material.gallery.slice(1).map((img) => (
                <div
                  ...
                  style={{
                    backgroundImage: `linear-gradient(130deg, rgba(26,26,26,0.25), rgba(26,26,26,0.08)), radial-gradient(circle at 70% 20%, rgba(255,255,255,0.2), transparent 48%), url('${img}')`,
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                  }}
                />
          ...
          <div className="rounded-2xl border border-pietra-sand/35 bg-white p-6">
            ...
            <h1 className="mt-3 text-3xl font-bold text-pietra-black">
              {material.name}
            </h1>

            <p className="mt-2 text-sm font-semibold uppercase tracking-wide text-pietra-black/60">
              {material.category}
            </p>

            <p className="mt-4 text-pietra-black/80">{material.description}</p>

            <p className="mt-4 text-lg font-semibold text-pietra-green">
              Desde Gs. consultar
            </p>
            ...
            <div className="mt-6 grid gap-2 sm:grid-cols-2">
              <a
                className="rounded-full bg-pietra-green px-4 py-3 text-center font-semibold text-white"
              ...
              <a
                className="rounded-full border border-pietra-black px-4 py-3 text-center font-semibold"
        ...
        <div className="grid gap-4 rounded-2xl border border-pietra-sand/35 bg-white p-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-2 text-sm text-pietra-black/75">
              Ideal para proyectos que buscan estética premium y funcionalidad
              técnica.
            </p>

            <h3 className="mt-4 font-semibold">Usos recomendados</h3>

            <ul className="mt-2 text-sm text-pietra-black/75">
          ...
          <div>
            ...
            <div className="mt-3 divide-y divide-pietra-sand/30 rounded-xl border border-pietra-sand/35">
              {material.specs.map((spec) => (
                ...
                >
                  <span className="font-medium text-pietra-black/70">
                    {spec.label}
                  </span>
                  <span className="font-semibold text-pietra-black">
                    {spec.value}
                  </span>
                </div>
              ))}
            ...
            <div className="mt-2 space-y-2">
              {material.faq.map((item) => (
                <details
                  key={item.q}
                  className="rounded-xl border border-pietra-sand/35 bg-pietra-ivory p-3 text-sm"
                >
