    ...
    <main className="pb-20 md:pb-0">
      ...
      <section className="mx-auto mt-4 max-w-7xl px-4">
        <div className="grid gap-2 overflow-x-auto pb-1 md:grid-cols-3">
          {[
            'Envío de medidas por WhatsApp',
            'Recomendación de material sin costo',
            'Instalación profesional garantizada',
          ].map((text) => (
            <div
              key={text}
              className="whitespace-nowrap rounded-full border border-pietra-sand/35 bg-white px-4 py-2 text-sm font-medium text-pietra-black"
            >
              {text}
            </div>
          ))}
      ...
      <section className="mx-auto mt-12 max-w-7xl px-4">
        <div className="mb-4 flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-pietra-black/75">
              Fichas comerciales diseñadas para cotización rápida.
            </p>
          </div>
        </div>
      ...
      <section className="mx-auto mt-12 max-w-7xl px-4">
        <h2 className="text-2xl font-bold text-pietra-black">
          Más elegidos por arquitectos y propietarios
        </h2>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-4">
          {highlightedMaterials.map((material) => (
            <article
              key={material.slug}
              className="rounded-2xl border border-pietra-sand/35 bg-white p-4"
            >
              <p className="text-xs font-semibold uppercase tracking-wide text-pietra-green">
                {material.category}
              </p>
              <h3 className="mt-1 font-semibold">{material.name}</h3>
              <p className="mt-2 text-sm text-pietra-black/70">
      ...
      <section className="mx-auto mt-12 max-w-7xl px-4">
        ...
        <p className="mt-2 text-pietra-black/70">
          Casos reales con resolución técnica y estética premium.
        </p>
